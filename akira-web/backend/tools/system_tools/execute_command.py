...
DANGEROUS_PATTERNS = [
    ...
    r"rmdir\s+/[sS]",
    r"del\s+/[fFsSqQ]",
    r"format\s+[A-Z]:",  # Narrowed: only blocks disk formatting, not /format:list
    r"mkfs",
    r"fdisk",
    r"dd\s+if=",
    ...
    # Targeted system path destruction (narrowed from broad path bans — allows read access)
    r"(del|rmdir|rd)\s+.*C:\\Windows",
    r"(del|rmdir|rd)\s+.*system32",
    r"(del|rmdir|rd)\s+.*C:\\Program Files",
    r"rm\s+-rf\s+(/etc|/usr|/var|/boot|/bin|/sbin|/root)",
]
