...
function formatHttpErrorBody(status, statusText, text) {
  ...
  if (!raw) {
    ...
    if (status === 502 || status === 503 || status === 504)
      return `Server unreachable (HTTP ${status}). Start the backend on port 8000 or fix the Vite proxy target.`;
  ...
  try {
    ...
    const hint =
      chatUrl.startsWith('/') && typeof window !== 'undefined'
        ? ' Start the backend (e.g. port 8000) so the Vite dev proxy can reach it.'
