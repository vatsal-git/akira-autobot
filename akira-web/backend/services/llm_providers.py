...
# Free OpenRouter models (from openrouter-demo.py)
...
# --- COMMENTED OUT: Previous paid models ---
# OPENROUTER_FALLBACK_MODEL_IDS = [
...
class LiteLLMProvider(BaseLLMProvider):
    """Unified provider via LiteLLM Router with native fallback handling."""
    ...
    def __init__(self, default_model_alias: Optional[str] = None):
        ...
        if not model_list:
            raise ValueError(
                "No models available. Set OPENROUTER_API_KEY env variable."
            )

        # Configure Router with native fallbacks
        ...
        self.logger.info("LiteLLM Router initialized with %d models, default: %s", len(model_list), self.default_alias)
    ...
    def invoke_streaming(
        ...
        alias = self._resolve_alias(model_override)
        self.model = alias
        route = self._resolved_litellm_route(alias)

        # OpenRouter-only: extra_body.reasoning is rejected by Bedrock
        extra_body = None
        ...
        # LiteLLM Router handles fallbacks natively
        response = self.router.completion(**kwargs)
        for chunk in _yield_normalized_stream_chunks(response, tools, self.logger):
            yield chunk
